...
class Character:
    ...
    def draw(self, screen: pygame.Surface):
        ...
        # ── ROTAÇÃO: inclinar sprite durante o pulo ────────
        if self.state == STATE_JUMP:
            # Subindo → inclina para frente (−15°); descendo → inclina para trás (+10°)
            tilt = -15 if self.vy < 0 else 10
            # Se virado para esquerda, inverte a inclinação
            if not self.facing_right:
                tilt = -tilt
            scaled_surf = pygame.transform.rotate(scaled_surf, tilt)
            sw = scaled_surf.get_width()
            sh = scaled_surf.get_height()
